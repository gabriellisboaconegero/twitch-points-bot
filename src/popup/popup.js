chrome.storage.sync.get(["channelsData", "watchingNow"], function(data){
    ...
    if (channels.length !== 0){
        ...
        for (const channel of channels) {
            document.getElementsByClassName(channel.name)[0].addEventListener("click", function(element){
                if (channel !== watching){
                    chrome.runtime.sendMessage({"msg": "select_and_move_to_tab", channel: channel.name, wasWatching:watching});
                }
            });
        }
    }
